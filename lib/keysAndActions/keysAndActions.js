...
const { colorize } = require('../escSequences/escSequences').colors;
  ...
  constructor(
    ...
    this.functions.createErr = (errMessage, position) => {
      ...
      return (
        moveRelY(-1) +
        moveAbsX(0) +
        colorize(0)(colorize(1, false, true)('Validation error:')) +
        ' ' +
        colorize(1)(errMessage) +
        moveRelY(1) +
  ...
  listeners = [
...

    {
      ...
      onKeyPress: ({ position, char, answers }) => {
        const { incrementCurrentPosition, print, redraw } = this.functions;
        const { withPin } = this.constants;
        const isUp = char === '\u001B\u005B\u0041';
        const newPosition = position + (isUp ? -1 : +1);
        const len = withPin ? Object.keys(answers).length : answers.length;
        if (newPosition >= 0 && newPosition < len) {
          incrementCurrentPosition(isUp ? -1 : +1);
          let result = moveRelY(len);
          for (let i = 0; i < len; i++)
            result += deleteAllLine() + moveRelY(-1);
          print(
            result + moveAbsX(0) + moveRelY(len) + redraw(newPosition, answers)
          );
        }
      },
    },

    {
      ...
      onKeyPress: ({ position, char, answers }) => {
        ...
        const len = Object.keys(answers).length;
        const isLeft = char === '\u001B\u005B\u0044';
        answers[Object.keys(answers)[position]] = !isLeft;
        let result = moveRelY(len);
        for (let i = 0; i < len; i++) result += deleteAllLine() + moveRelY(-1);
        print(result + moveAbsX(0) + moveRelY(len) + redraw(position, answers));
      },
    },
  ];
}
