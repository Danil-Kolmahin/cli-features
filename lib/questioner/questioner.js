...
class Questioner {
  ...
  defTruthyAnswers = [['yes'], ['no']]; // will be changed in 2.0

  checkYesOrNo = (str, defaultAnswer) => {
    // will be changed in 2.0
    const { defTruthyAnswers } = this;
    const input = str.toLowerCase();
    const check = (arr) =>
      arr.some((word) => word.startsWith(input) || input.startsWith(word));
    const isPositive = check(defTruthyAnswers[0]);
    const isNegative = check(defTruthyAnswers[1]);
  ...
  alternativeQuestion = async (
    ...
    const redraw = (selectedPosition, answers) => {
      ...
      if (maxLength === 1) valuesArray = [valuesArray[selectedPosition]];
      else if (len > maxLength || isRounded) {
        if (selectedPosition <= Math.ceil(maxLength / 2) - 1 && !isRounded)
          valuesArray = [
            ...valuesArray.slice(0, maxLength - 1),
            elseElementsSym,
          ];
        else if (
